...
local State = {
    ...
    autoFarm = false,
    farmSkills = true,
    farmClick = false, -- physical mouse1click â€” keep off so you can use your mouse
...
--[[
  ...
  Pad fallback uses NEAREST pad to you across aliases â€” never average / never
...
local function farmClick()
    -- Physical OS click â€” avoid while you want free mouse control
...
do
    ...
    s1:Toggle("Physical M1 Click", false, function(v)
        State.farmClick = v
        if v then
            notify("Farm", "M1 uses real cursor â€” leave OFF to free mouse", "bad")
        end
    end)
