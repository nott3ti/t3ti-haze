...
-- M1 tool: Auto â†’ fruit tool â†’ fighting style tool â†’ held â†’ Combat/Katana
...
--[[
  ...
  Works for ANY fruit â€” name heuristics + learned at runtime.
  Default OFF in farm inventory so they don't break camera mid-farm.
]]
-- Conservative name hints â€” runtime learning covers the rest per fruit
...
-- Match a quest option to the current farm target (e.g. Revolutionary â†’ Revolutionary Elite @ Base)
...
local function hasActiveKillQuest()
    ...
    -- AmountMobs missing/unreadable â€” use QuestHandler, then objective label
    local q = questValues()
...
local function ensureBestQuest()
    ...
    -- No quest / wrong quest / completed â†’ accept best for level
    local ok, err = acceptQuest(best)
    if ok then
        requestFarmRepath()
        State.status = "accepted Â· " .. tostring(want)
        return true, want
    end
    -- Server "AlreadyOnQuest" with blank values â€” trust label target
...
local function allQuestPads(keys)
    ...
    for _, k in ipairs(keys) do
        for _, p in ipairs(findQuestSpawnPads(k)) do
            -- Cached pads have part=nil â€” never use nil as table key
            local id = p.part
...
--[[
  ...
  Stuck = sideways nudge only â€” never +Y sky hop.
]]
...
local function setFlyHumanoid(hum, on)
    ...
    pcall(function()
        if on then
            -- PlatformStand only â€” avoid Physics state (breaks camera subject)
...
local function bvFlyTo(goal, opts)
    ...
    -- Yaw-only gyro â€” full MaxTorque tilts the body and breaks camera
    ...
    while token == _questFlyToken and hrp.Parent and UI.alive and (tick() - t0) < maxT do
        ...
        -- stuck: sideways around obstacle (XZ), pull Y toward goal only â€” no sky hop
        if stuckFor >= 6 then
            stuckFor = 0
            sideSign = -sideSign
...
-- Always keep noclip during farm holds â€” toggling it off mid-fight = server rubberband.
...
local function farmHoldAt(goal, lookAt, _mode)
    ...
    if not bg then
        ...
        -- yaw only â€” full torque + CFrame snaps = camera break / hitch
        bg.MaxTorque = Vector3.new(0, 4e10, 0)
        bg.P = 12000
        bg.D = 600
        bg.Parent = hrp
...
local function setVirtualAim(pos, targetInst)
    ...
    -- Throttle mouse remote â€” spam freezes / desyncs camera scripts
...
-- Face via yaw only through BodyGyro when farming â€” never CFrame-snap HRP (camera break).
local _lastFaceAt = 0
...
local function waitOutCutscene(maxWait)
    ...
    State.status = "cutscene Â· waiting"
    -- Do NOT restoreCamera while Scriptable â€” Sea Rift's Heartbeat owns the cam.
    ...
    if inCutscene() then
        -- stuck Scriptable (anim never ended) â€” force unlock
        restoreCamera()
    else
        destroyCutsceneRigs()
        pcall(function()
...
-- After casting a skill, if camera flips â†’ learn it as cinematic for this session
...
-- Perch near pad â†’ aim â†’ cast selected (or Auto) skill. Any fruit.
...
do
    ...
    s3:Label("Safe perch â†’ aim pad â†’ Stats skill (any fruit)")
    s3:Button("Pad Clear (test)", function()
        task.spawn(function()
            local ok, err, target, pad, _, skill = padClearSkill()
            if ok then
                notify("Pad Clear", tostring(skill) .. " Â· " .. tostring(target), "good")
            else
...
do
    ...
    s1:Label("BV-noclip travel Â· stable quest hops Â· no island TP")
...
do
    ...
    sWep:Label("Fruit: " .. tostring(currentFruitName() or "none")
        .. " Â· Style: " .. tostring(currentFightingStyle() or "none"))
    sWep:Dropdown("M1 Tool", m1Choices, State.m1Tool, function(v)
        State.m1Tool = v
        local tip = v
        if v == "Auto" then
            tip = "fruitâ†’styleâ†’held Â· " .. tostring(currentFruitName() or "?")
        end
        notify("M1", tip, "good")
    end)
    sWep:Label("Auto = fruit tool â†’ fighting style â†’ held â†’ Combat")

    local s2 = tab:Section("Skill Inventory")
    s2:Label("Tick farm skills Â· cinematic moves default OFF")
...
local _farmIslandGoal = nil -- Vector3: commit to island hop (stops NPCâ†”island rubberband)
...
local function resolveFarmTarget(targetName, hrp)
    -- Stick to the same NPC until dead / too far â€” but ONLY if it still matches the quest
...
-- Auto Farm: fly in once â†’ HOLD stiff beside target â†’ M1
task.spawn(function()
    local lastFly = 0
    local lastPanel = 0
    while UI.alive do
            ...
            if State.autoFarm then
                ...
                if targetName and hrp then
                    ...
                    if npc and pos then
                        ...
                        -- Near the fight: ONLY soft hold â€” full bvFlyTo causes killâ†’spawn bounce
                ...
                -- Stuck Sea Rift Heartbeat (Scriptable never ends) â€” unlock after ~8s
                if inCutscene() then
        ...
        if not okIter then
            State.status = "farm err Â· " .. tostring(errIter):sub(1, 40)
...
local function revealUI()
    ...
    if State.autoFarm then
        requestFarmRepath()
        notify("T3ti", "Auto Farm resumed â€” RCtrl menu", "good")
    else
        notify("T3ti", "Haze helper ready â€” enable Auto Farm to move", "good")
    end
    print("[T3ti] Haze Seas helper loaded Â· autoFarm=" .. tostring(State.autoFarm))
end
