...
local function padNameMatchesKey(padName, key)
    ...
    -- Exact / pad contains full quest name (Snow Harpy2400 · snow harpy)
    -- NEVER key:find(pbase) — "skull pirate" would match pad "Pirate"
...
local function findQuestSpawnPads(targetName)
    ...
    if #pads == 0 and Persist.padCache then
        for base, c in pairs(Persist.padCache) do
            ...
            -- Must match by pad name — ignore quest-keyed junk entries
...
local function npcNameMatches(instName, keys)
    ...
    n = n:gsub("%d+$", ""):gsub("%s+$", "")
    -- digit-only names become "" — never match (avoids empty:find / false hits)
...
local function cachePad(targetName, pad)
    ...
    local key = tostring(targetName or ""):lower()
    if key == "" then return end
    -- Never cache a pad whose name doesn't match the quest (old bug: Pirate → Skull Pirate)
...
-- Quest giver Location strings → NPC Zones / Islands / Spawn Setter names
...
local function locationHintsForTarget(targetName)
    ...
    -- Name bias for split islands (Snow/Harpy/Corrupt → cold; Vergo → hot already in hints)
...
local function placeStandFromInstance(inst, label)
    ...
    -- Live humanoids first (already streamed chunk) — any NPC gets you into range
    ...
    -- Island / zone geometry — prefer away from dock so far-side bosses stream in
...
-- Any Observation pads on the quest's islands (even wrong mob) → stream range
...
-- Locate waypoint above Observation spawn (travel only — never a farm perch)
...
local function onFarmQuestChanged(targetName)
    ...
    clearFarmArena()
    requestFarmRepath()
    -- Lock travel to spawn-area locator (pad/island) — locate only, not a farm perch
                ...
                if targetName and hrp then
                    ...
                    if islandHopping then
                        ...
                        elseif npc and dist then
                            -- Mob exists anywhere — leave pad path and fight
                            _farmIslandGoal = nil
                            _farmIslandName = nil
                            islandHopping = false
                        else
                    ...
                    if npc and pos then
                        ...
                        if locating then
                            ...
                            if dist0 < LOCATE_ARRIVE then
                                -- Located spawn area — drop pad goal; only fight live NPCs from here
                                _farmIslandGoal = nil
                                _farmIslandName = nil
                                locating = false
                            else
                        ...
                        if not locating then
                            ...
                            elseif locateStand and locateDist and locateDist <= LOCATE_REPATH then
                                ...
                                stopFarmHold()
                                State.status = string.format("scan · %s", tostring(targetName))
                                perch = nil
                            elseif locateStand then
                                ...
                                stopFarmHold()
                                State.status = "no locate · " .. tostring(targetName)
                            end
                        end
                        ...
                        if perch then
                            ...
                            elseif isLocate then
                                ...
                                -- Respawn wait at last combat — not on pad
                                _questFlyToken = _questFlyToken + 1
                                farmHoldAt(perch, perch, "travel")
                                State.status = string.format(
                                    "wait · %s · %s",
