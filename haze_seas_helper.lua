...
local State = {
    ...
    farmRange = 14, -- engage / stay near target (M1 needs ~5–10 studs)
...
-- Loops
--------------------------------------------------------------------
-- Combat stand: slightly above feet, not sky-hover (M1 needs ~5–10 studs)
...
-- Auto Farm: find live NPC → BV into melee → tool Activate + selected skills.
            ...
            if targetName and hrp then
                ...
                if npc and pos then
                    ...
                    if dist > melee + 1.5 and tick() - lastFly > 0.25 then
                        lastFly = tick()
                        State.status = string.format("close · %.0fd", dist)
                    ...
                    -- No live NPC streamed — travel to pad / stand for this target
                    clearVirtualAim()
